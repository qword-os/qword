...
void init_idt(void) {
    for (size_t vec = 0; vec < 256; vec++) {
        register_interrupt_handler(vec, int_handler, 0x8e);
    }
    ...
    register_interrupt_handler(IPI_ABORTEXEC, ipi_abortexec, 0x8e);

    for (size_t i = 0; i < 16; i++) {
        register_interrupt_handler(0x90 + i, apic_nmi, 0x8e);
    } 
    ...
    for (size_t i = 0; i < 8; i++) {
        register_interrupt_handler(0xa0 + i, pic0_generic, 0x8e);
    }
    for (size_t i = 0; i < 8; i++) {
        register_interrupt_handler(0xa8 + i, pic1_generic, 0x8e);
    }
